...
pthread_i * ltl_search(int blastid)
{
    ltl * ltl_tmp_node;

    if (!blastid)
        blastid = blast_thread_myid();
    ...
    while (ltl_tmp_node)
    {
        ...
        {
            if (ltl_tmp_node->ltcb->pthread == blastid)
            {
                if (0 != pthread_mutex_unlock(&ltl_mutex))
                    goto fail;
...
int ltl_delete(int blastid)
{
    ...
    while (ltl_del_node)
    {
        if (ltl_del_node->ltcb->pthread == blastid)
        {
