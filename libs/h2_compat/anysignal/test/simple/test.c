...
h2_anysignal_t any_threads, any_done;
int t0id,t1id;
...
//	h2_init(NULL);
	...
	h2_anysignal_wait(&any_threads, 0x3);
	if(any_threads.waiting != 0) { FAIL("anysignal didn't block while waiting"); }

	h2_anysignal_set(&any_done, 0x1);
	h2_anysignal_set(&any_done, 0x2);
	h2_anysignal_set(&any_done, 0x4);
	h2_anysignal_set(&any_done, 0x8);
	h2_anysignal_set(&any_done, 0xf);
	sig = h2_anysignal_get(&any_done);
