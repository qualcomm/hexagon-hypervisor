...
typedef qurt_fd_table_t pthread_id_table_t;

extern pthread_id_table_t pthread_id_table;

#define pthread_id_table_init qurt_fd_table_init_with_mem

#define pthread_id_new(handle) \
    qurt_fd_table_add(&(pthread_id_table), (handle))

#define pthread_id_delete(obj) \
    qurt_fd_table_remove(&(pthread_id_table), (qurt_obj_t)(obj))

#define pthread_id_is_valid(obj) \
    qurt_fd_table_is_valid_obj(&(pthread_id_table),(obj))

#define pthread_id_get_handle(m_obj) \
    qurt_fd_table_get_handle(&(pthread_id_table), (m_obj))

/* QURT Signal bit used for POSIX */
...
/* QURT signo 3-8 are used for timer */
