...
:cfunc:`H2K_handle_error()` takes an error and applies the appropriate handling method.
...
NULL.  If the error handler is NULL, we call :cfunc:`H2K_fatal_thread()`.  If the 
error was caused by an error handler, we call :cfunc:`H2K_fatal_thread()`.  
