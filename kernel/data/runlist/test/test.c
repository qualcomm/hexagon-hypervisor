...
int main() 
{
	...
	H2K_gp->runlist_valids = 0xdeadbeef;
	H2K_gp->runlist[0] = &a;
	H2K_runlist_init();
	if (H2K_gp->runlist_valids != 0) FAIL("runlist_init failed to set valids");
	...
	if (H2K_gp->runlist[2] != &a) FAIL("runlist_append failed (0) ");
	if ((H2K_gp->runlist_valids & (1<<2)) == 0) FAIL("runlist_append failed (1) ");
	if (H2K_gp->runlist[2]->next != NULL) FAIL("runlist_append failed (2) ");
	if (H2K_runlist_worst_prio_TB() != 2) FAIL("runlist worst prio (4) ");

	H2K_runlist_push_TB(&b);
	if (H2K_gp->runlist[2] != &b) FAIL("runlist_append failed (5)");
	if ((H2K_gp->runlist_valids & (1<<2)) == 0) FAIL("runlist_append failed (6)");
	if (H2K_gp->runlist[2]->next != &a) FAIL("runlist_append failed (7)");
	if (H2K_gp->runlist[2]->next->next != NULL) FAIL("runlist_append failed (9)");
	...
	if (H2K_gp->runlist[2] != &b) FAIL("ready_remove failed");
	if ((H2K_gp->runlist_valids & (1<<2)) == 0) FAIL("ready_remove failed");
	if (H2K_gp->runlist[2]->next != NULL) FAIL("ready_remove failed");
	if (H2K_runlist_worst_prio_TB() != 2) FAIL("Ready best prio");

	H2K_runlist_remove_TB(&b);
	if (H2K_gp->runlist[2] != NULL) FAIL("ready_remove failed");
	if (H2K_gp->runlist_valids != 0) FAIL("ready_remove failed");
