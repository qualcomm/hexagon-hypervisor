...
u32_t find_tlb_index_last_used(void) {
    u32_t found_tlb_index = H2K_gp->last_tlb_index;  
    for (u32_t index = 0; index < H2K_gp->last_tlb_index; index++) {
...
int main()
{
    ...
    H2K_gp->tlb_size = 128;
    H2K_gp->last_tlb_index = H2K_kg.tlb_size - 1;
    H2K_gp->pinned_tlb_mask = (~0ULL) << ((H2K_gp->last_tlb_index+1) & 0x3F);
    
	u32_t tlb_index_last_used = find_tlb_index_last_used();
	if (tlb_index_last_used >= H2K_gp->last_tlb_index) {
	...
	if (tlb_index >= H2K_gp->last_tlb_index) {
		FAIL("TLB free index not set to attempt invalidate");
	}
	
	H2K_mem_tlb_invalidate_va(va, 1, asid, &H2K_tc); // TBD
	
	u32_t tmp = H2K_mem_tlb_probe(va, asid);
