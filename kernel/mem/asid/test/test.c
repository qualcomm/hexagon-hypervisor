...
void check_max()
{
	...
	for (i = 0; i < MAX_ASIDS; i++) {
		if ((tmp = H2K_asid_table_inc((u32_t)(&bases[i]), H2K_ASID_TRANS_TYPE_LINEAR, H2K_ASID_TLB_INVALIDATE_FALSE)) < 0) {
...
void check_invalid()
{
	...
	H2K_asid_entry_t entry;
	s32_t tmp;
	if ((tmp = H2K_asid_table_inc((u32_t)(&bases[0]), H2K_ASID_TRANS_TYPE_LINEAR, H2K_ASID_TLB_INVALIDATE_FALSE)) < 0) {
		FAIL("Couldn't alloc max ASIDs");
	}
	bases[0] = tmp;
	entry = H2K_mem_asid_table[bases[0]];
