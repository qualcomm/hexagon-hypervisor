...
unsigned long vm_setup(unsigned int num_cpus, short num_ints, u32_t trans, unsigned long trapmask, translation_type pt) {
	...
	if (pt == H2K_ASID_TRANS_TYPE_OFFSET) {
		...
		ret = h2_config_vmblock_init(vm, SET_FENCES, 0x01000000, 0xfe000000);
		if (ret != vm) {
...
volatile unsigned int t32_vm_loaded_flag = 0;
volatile unsigned int t32_vm_entry;
...
unsigned long boot_vm() {
	...
	PRINTF("vm set up foo=%x\n",H2K_ASID_TRANS_TYPE_OFFSET);
	while (t32_vm_loaded_flag == 0) /* SPIN */;
	if (h2_vmboot((void *)t32_vm_entry, (void *)0x20000000, t32_vm_entry, vm_best_prio, newvm) == -1) FAIL("vmboot");
	t32_vm_loaded_flag = 0;
	return newvm;
}
...
int main()
{
	unsigned long new_vm;
	unsigned int status;
	unsigned int cpus;
	PRINTF("t32boot: H2 started\n");
	...
	do {
		...
		do {  // wait for all child VM cpus to vmstop
			h2_vmtrap_wait();
			status = h2_vmstatus(VMOP_STATUS_STATUS, new_vm);
			PRINTF("VM %lu status %x\n", new_vm, status);
			cpus = h2_vmstatus(VMOP_STATUS_CPUS, new_vm);
