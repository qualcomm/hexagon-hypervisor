...
execute the trap, we call :c:func:`H2K_fatal_thread()`.  If a fast interrupt cannot
execute the trap, we call :c:func:`H2K_fatal_kernel()`.
...
Several traps handlers may point to :c:func:`H2K_thread_id()`, which 
is used wherever empty traptab spaces exist.  The table is as follows:

 - 0x00 :c:func:`H2K_trap_angel()`: dummy handler for ANGEL semihosting
 - 0x01 :c:func:`H2K_thread_id()`: Get the thread ID
 - 0x02 :c:func:`H2K_futex_wait()`: Wait on a futex
 - 0x03 :c:func:`H2K_futex_resume()`: Wake threads waiting on a futex
 - 0x04 :c:func:`H2K_thread_create()`: Create a new thread
 - 0x05 :c:func:`H2K_thread_stop()`: Stop and recycle the current thread
 - 0x06 :c:func:`H2K_cputime_get()`: Get CPU time
 - 0x07 :c:func:`H2K_thread_id()`: Get the thread ID
 - 0x08 :c:func:`H2K_register_fastint()`: Register a fast interrupt handler
 - 0x09 :c:func:`H2K_prio_set()`: Change priority
 - 0x0A :c:func:`H2K_prio_get()`: Get priority
 - 0x0B :c:func:`H2K_popup_wait()`: Block, waiting on an interrupt
 - 0x0C :c:func:`H2K_sched_yield()`: Yield to another thread at the same priority
 - 0x0D :c:func:`H2K_thread_id()`: Get the thread ID
 - 0x0E :c:func:`H2K_thread_id()`: Get the thread ID
 - 0x0F :c:func:`H2K_thread_id()`: Get the thread ID
 - 0x10 :c:func:`H2K_pcycles_get()`: Get processor cycle count
 - 0x11 :c:func:`H2K_thread_id()`: Get the thread ID
 - 0x12 :c:func:`H2K_tid_set()`: Set software thread ID
 - 0x13 :c:func:`H2K_tid_get()`: Get software thread ID
 - 0x14 :c:func:`H2K_futex_lock_pi()`: Priority Inheritance Futexy-Mutex Lock
 - 0x15 :c:func:`H2K_futex_unlock_pi()`: Priority Inheritance Futexy-Mutex Unlock
 - 0x16 :c:func:`H2K_timer_trap()`: Timer operations
 - 0x17 :c:func:`H2K_thread_id()`: Get the thread ID
 - 0x18 :c:func:`H2K_thread_id()`: Get the thread ID
 - 0x19 :c:func:`H2K_thread_state()`: Read thread context
 - 0x1A :c:func:`H2K_trap_info()`: Get system configuration info
 - 0x1B :c:func:`H2K_waitcycles_get()`: Get waitcycles for a hardware thread
 - 0x1C :c:func:`H2K_vmop()`: Boot/query/free a VM
 - 0x1D :c:func:`H2K_trap_pmuctrl()`: Adjust PMU Configuration
 - 0x1E :c:func:`H2K_trap_config()`: Adjust Kernel Configuration
 - 0x1F :c:func:`H2K_trap_hwconfig()`: Adjust Hardware Configuration
...
.. c:function:: void TH_do_trap(H2K_thread_context *src, H2K_thread_context *dest, u32_t trapnum)

This function will load the appropriate registers from the src thread context, set
SGP to the storage pointed to by `dest`, and call :c:func:`H2K_handle_trap0()` with the
...
.. c:function::  void TH_check_trap(H2K_thread_context *src, H2K_thread_context *dest)
