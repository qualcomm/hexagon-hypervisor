...
void thread0(int thread)
{
...
#ifdef SIMULATOR_PMU_SUPPORT
	if (myabs((h2_pmu_getreg(H2_PMUCNT0)/2) - SPINS) > (SPINS/2)) FAIL("Unexpected time 0");
	if (myabs((h2_pmu_getreg(H2_PMUCNT1)/2) - SPINS) > (SPINS/2)) FAIL("Unexpected time 1");
	if (myabs((h2_pmu_getreg(H2_PMUCNT2)/2) - SPINS) > (SPINS/2)) FAIL("Unexpected time 2");
	if (myabs((h2_pmu_getreg(H2_PMUCNT3)/2) - SPINS) > (SPINS/2)) FAIL("Unexpected time 3");
#endif
	puts("c.ok");
	...
	h2_sem_down(&sem_ret);
	//	delay();
	if (h2_pmu_getreg(H2_PMUEVTCFG) != 0) FAIL("unexpected pmu value/l");
