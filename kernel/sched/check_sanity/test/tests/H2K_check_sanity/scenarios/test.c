...
static void setup(struct scenario *scenario)
{
	...
	H2K_runlist_init();
	H2K_lowprio_init();
	/* The full boot path (H2K_init_setup) normally enables the hardware
	 * reschedule interrupt.  This is a standalone unit test that never runs
	 * that path, so enable SCHEDCFG here or the BESTWAIT comparator stays off
	 * and no reschedule interrupt is ever raised. */
	...
	asm volatile ("isync");
	asm volatile ("stid = %0; isync" : : "r"((u32_t)worst_running_prio << 16));
	H2K_clear_ipend(0xffffffff);
