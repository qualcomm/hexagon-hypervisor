                 ...
                 const struct timespec *abs_timeout)
{
    ...
    if (thread)
    {
        ...
        qurt_anysignal_set(&ltcb->sigs, POSIX_MQ_NOTIF_MASK);
    }
                        ...
                        const struct timespec *abs_timeout)
{
	...
	{
        sig_recv = qurt_anysignal_wait(&ltcb->sigs, POSIX_MQ_NOTIF_MASK);
	}
    ...
    {
        qurt_anysignal_clear(&ltcb->sigs, POSIX_MQ_NOTIF_MASK);
            ...
            const sigset_t *restrict sigmask)
{
    ...
    else
    {
        while (retry)
        {
            ...
            ret = qurt_anysignal_wait(&ltcb->sigs, POSIX_MQ_NOTIF_MASK | POSIX_SIGNAL_MASK);
