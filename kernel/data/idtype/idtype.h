...
#define H2K_ID_CPUIDX_MASK (((1 << H2K_ID_CPUIDX_BITS) - 1) << H2K_ID_RSVD_BITS)
#define H2K_ID_VMIDX_BITS 6
#define H2K_ID_MAX_VMS (1<<(H2K_ID_VMIDX_BITS))
#define H2K_ID_MAX_CPUS (1<<(H2K_ID_CPUIDX_BITS))

#if ((H2K_ID_RSVD_BITS+H2K_ID_VMIDX_BITS+H2K_ID_CPUIDX_BITS) != 32)
...
typedef union {
	u32_t raw;
	struct {
		u32_t reserved:H2K_ID_RSVD_BITS;
